...
import openai
from llama_index.anthr import AnthropicMultiModal
...
def load_data():
    with st.spinner(text="Loading and indexing the docs hang tight! This should take 5 minutes."):
        ...
        llm = OpenAI(model="gpt-3.5-turbo-0125", temperature= 0.5, system_prompt='You are a manufacturing assistant. Recommend the product as close as possible to the user request based on all specifications.')
        ...
        parser = LlamaParse(api_key= LLAMA_CLOUD_API_KEY,  # can also be set in your env as LLAMA_CLOUD_API_KEY
                            ...
                            Find information for each component and be ready to provide it. Be able to match up each product with all of its corresponding specifications across the table. """,
                            verbose=True
        )
...
if "chat_engine" not in st.session_state.keys(): # Initialize the chat engine
       # st.session_state.chat_engine = index.as_chat_engine(chat_mode="condense_question", verbose=True)
        ...
        memory=memory,
        context_prompt=(
        "You are a manufacturing chatbot assistant, able to provide recommendations about products whether or not it is possible to manufacture them based on the provided information. You can give product specs and compare them to others. Recommend the product as the one with the closest specifications based on the request."
        "Here are the relevant documents for the context:\n" 
        "{context_str}"
        "\nInstruction: Use the previous chat history, or the context above, to interact and help the user find the best recommendation based on the user's request."
        ),
        verbose=True,
        )
